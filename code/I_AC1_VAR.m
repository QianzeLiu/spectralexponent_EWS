...
set(gca,'XAxisLocation','top','YAxisLocation','right','XDir','reverse','xlim',[-5 0],'XTicklabels',[eval(vpa(-0.5:0.5/5:0 , 2))],'YTick',[]);
...
hold on
plot(xt,mean(I),'k','linewidth',4),xlabel(ax1,'Control parameter - M','FontWeight','bold'),xlabel(ax2,' '),set(ax2 ,'xticklabels', [])
xline(xl,'--k','linewidth',3)
ylabel('Sea Ice - I(t)','FontWeight','bold') ,xlim([0 100]),
text(-0,3,'(a)','FontWeight','bold',FontSize=24)
...
set(gca ,'xticklabels', [])
text(-0,-4.2,'(b)','FontWeight','bold',FontSize=24),xlim([0 100]),%ylim([-5.8,-4.5])

text(60,mp_burg(xl/dt-W),['\tau = ',num2str(tau1)],'color','k','FontWeight','bold',FontSize=14)
...
text(60,meanAC1(xl/dt-W),['\tau = ',num2str(tau2)],'color','k','FontWeight','bold',FontSize=14)
...
text(-0,0.05,'(d)','FontWeight','bold',FontSize=24)
xlim([0 100])
text(1,meanVAR(1),'-sliding window-',FontSize=12)
axis normal
text(60,meanVAR(xl/dt-W),['\tau = ',num2str(tau3)],'color','k','FontWeight','bold',FontSize=14)
...
legend(['Spectral exponent  AUC = ',num2str(auc(1))], ...
    ...
    ['Variance                  AUC = ',num2str(auc(3))],FontSize=12,Location="best")
legend('boxoff')
set(gca,'fontweight','bold','linewidth',4,FontSize=19)
